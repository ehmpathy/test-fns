...
given.only = <F extends () => void>(
  ...
  const name = `given: ${desc}`;
  describeStack.push(name);
  (globals().describe as any).only(name, fn);
  describeStack.pop();
};
given.skip = <F extends () => void>(
  ...
  const name = `given: ${desc}`;
  describeStack.push(name);
  (globals().describe as any).skip(name, fn);
  describeStack.pop();
};
given.skipIf =
  (condition: boolean) =>
...
const then: Test = ((...input: TestInput<void>): void => {
  ...
  // check if we're in a repeatably context (exact or ancestor path)
  const repeatableCtx = findRepeatableContext(path);

  // if not in repeatably context, use original passthrough (no wrapper overhead)
  if (!repeatableCtx) {
    globals().test(name, testFn);
    return;
  }

  // in repeatably context: wrap with skip-on-success and failure detection
  globals().test(name, async function (this: any) {
    // use captured context (already resolved via findRepeatableContext)
    const ctx = repeatableCtx;

    // skip if prior attempt succeeded
    if (ctx?.anyAttemptPassed) {
      console.log(`    🫧  [SKIPPED] prior attempt passed`);
      // vitest 3.1+: use test context skip if available
    ...
    if (testFn) {
      try {
        ...
        if (ctx) ctx.thisAttemptFailed = true;
